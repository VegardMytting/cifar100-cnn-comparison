...
def train_cnn(model_name, optimizer_name, learning_rate, use_transfer_learning = False, cnn_activation_function = None):
  ...
  for run in tqdm(range(RUNS), desc="Runs", position=0, leave=False):
    ...
    precision = precision_score(run_labels, run_predictions, average='macro', zero_division=0) * 100
    recall = recall_score(run_labels, run_predictions, average='macro', zero_division=0) * 100
    f1 = f1_score(run_labels, run_predictions, average='macro', zero_division=0) * 100
  ...
  std_precision = statistics.stdev(precisions)
  ...
  console.print(f"[#e5c07b]![/#e5c07b] Accuracy:  \t{mean_acc:.2f} % ± {std_acc:.2f}")
  console.print(f"\n[#e5c07b]![/#e5c07b] Precisions: \t\t{precisions}")
  console.print(f"[#e5c07b]![/#e5c07b] Precision: \t{mean_precision:.2f} % ± {std_precision:.2f}")
  console.print(f"\n[#e5c07b]![/#e5c07b] Recalls: \t\t{recalls}")
  console.print(f"[#e5c07b]![/#e5c07b] Recall:    \t{mean_recall:.2f} % ± {std_recall:.2f}")
  console.print(f"\n[#e5c07b]![/#e5c07b] F1_Scores: \t\t{f1_scores}")
  console.print(f"[#e5c07b]![/#e5c07b] F1-Score:  \t{mean_f1:.2f} % ± {std_f1:.2f}")
  ...
  plt.close()
  console.print(f"[#e5c07b]![/#e5c07b] Confusion matrix saved to: confusion_matrix_{model_name.replace(' ', '_')}.png")
  
  # console.print(f"\n[#e5c07b]![/#e5c07b] Classification Report:")
