...
def save_epoch_accuracies_to_csv(epoch_accuracies, epoch_losses, model_name, optimizer_name, learning_rate, use_transfer_learning=None, cnn_activation_function=None):
  timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
  
  if model_name == "Custom CNN":
    filename = f"epoch_accuracies_{model_name}_{cnn_activation_function}_{optimizer_name}_lr{learning_rate}_{timestamp}.csv"
  else:
    transfer_str = "transfer" if use_transfer_learning else "no_transfer"
    filename = f"epoch_accuracies_{model_name}_{transfer_str}_{optimizer_name}_lr{learning_rate}_{timestamp}.csv"
  
  with open(filename, 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['epoch', 'mean_acc', 'mean_loss'])
    
    for epoch_idx, (accuracies_at_epoch, epoch_losses_at_epoch) in enumerate(zip(epoch_accuracies, epoch_losses)):
      mean_acc = statistics.mean(accuracies_at_epoch)
      mean_loss = statistics.mean(epoch_losses_at_epoch)
      writer.writerow([epoch_idx + 1, f"{mean_acc:.4f}", f"{mean_loss:.6f}"])

  console.print(f"[#e5c07b]![/#e5c07b] Saved epoch accuracies to: [#98c379]{filename}[/#98c379]")
  return filename

def train_cnn(model_name, optimizer_name, learning_rate, use_transfer_learning = False, cnn_activation_function = None):
  accuracies = []
  epoch_accuracies = [[] for _ in range(EPOCHS)]
  ...
  for run in tqdm(range(RUNS), desc="Runs", position=0, leave=False):
    ...
    for epoch in tqdm(range(EPOCHS), desc=f"Training (Run {run+1})", position=1, leave=False):
      ...
      epoch_accuracies[epoch].append(epoch_acc)
      
      epoch_loss = running_loss / len(trainloader)
      epoch_losses[epoch].append(epoch_loss)
  ...
  save_epoch_accuracies_to_csv(epoch_accuracies, epoch_losses, model_name, optimizer_name, learning_rate, use_transfer_learning, cnn_activation_function)
